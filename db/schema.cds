...
annotate Customer with @assert.unique :
{
    email : [ email ],
};
...
entity GiftVoucher : cuid
{
    ...
    batch : Integer default 80126001;
    ...
    assignGiftVouchers : Association to many AssignGiftVoucher on assignGiftVouchers.giftVoucher = $self;
}

annotate GiftVoucher with @assert.unique :
{
    coupon : [ coupon ],
};
...
annotate Bill with @assert.unique :
{
    no : [ no ],
};
...
entity GVHeader : cuid
{
    ...
    gr_type : Association to one GVRTypes default 'CI'
        @mandatory;
    campaign : Association to one MallCampaign on campaign.campaign = $self.campaign.campaign;
    customer : Association to one Customer on customer.email = $self.customer.email
        @mandatory;
    employee : Association to one Employee on employee.code = $self.employee.code
        @mandatory;
    bills : Composition of many Bill on bills.gVHeader = $self
        @mandatory;
    giftVouchers : Composition of many AssignGiftVoucher on giftVouchers.gVHeader = $self
        @mandatory;
}

annotate GVHeader with @assert.unique :
{
    gv_no : [ gv_no ],
};
